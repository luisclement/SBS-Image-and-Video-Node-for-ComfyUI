### Luis Clement modeified version to use Pytorch acceleration from CPU ##############
import torch
...
class SBS_V2_by_SamSeen:
    ...
    def process(self, base_image, depth_scale, blur_radius, invert_depth=False, mode="Cross-eyed"):
        ...
        for b in range(B):
            ...
            # Set flip for mode
            fliped = 0 if mode == "Parallel" else W
            ...
            current_image = current_image.permute(2, 0, 1).unsqueeze(0)  # [1, C, H, W]
            shifted = F.grid_sample(current_image, shift_grid.unsqueeze(0), mode='nearest', padding_mode='border', align_corners=True).squeeze(0).permute(1, 2, 0)  # [H, W, C]

            # Apply to the right/left view
            sbs_image[:, fliped:, :] = shifted
            ...
            for shift in range(1, max_shift + 1):
                ...
                shifted_shift = F.grid_sample(current_image, shift_grid.unsqueeze(0), mode='nearest', padding_mode='border', align_corners=True).squeeze(0).permute(1, 2, 0)
                sbs_image[:, fliped:, :] = torch.where(shifted_shift > 0, shifted_shift, sbs_image[:, fliped:, :])
